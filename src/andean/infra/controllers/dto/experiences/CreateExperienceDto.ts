...
export class ExperienceBasicInfoDto {
	...
	@IsEnum(ExperienceDurationUnit)
	durationUnit!: ExperienceDurationUnit;

	@ApiPropertyOptional({
		description:
			'Duración en horas cuando durationUnit=HOURS (en DAYS puede omitirse)',
		example: 6,
	})
	@IsInt()
	@Min(1)
	...
	@ApiProperty({
		description: 'Indica si incluye recojo',
		example: true,
	})
	@IsBoolean()
	...
	@ApiProperty({
		description: 'Indica si incluye alojamiento',
		example: true,
	})
	@IsBoolean()
	...
	@ApiProperty({
		description: 'Indica si incluye retorno',
		example: true,
	})
	@IsBoolean()
