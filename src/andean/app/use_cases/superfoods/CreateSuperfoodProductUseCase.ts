...
export class CreateSuperfoodProductUseCase {
  ...
  async handle(dto: CreateSuperfoodDto): Promise<SuperfoodProduct> {
    ...
    if (dto.baseInfo.ownerType === SuperfoodOwnerType.SHOP) {
      ...
      if (!shopFound) {
        throw new NotFoundException(
          `Shop con ID ${dto.baseInfo.ownerId} no encontrado`,
        );
      }
    }
    ...
    // 4. Validar que no hay variantes duplicadas solo si existen
    if (dto.variants && dto.variants.length > 0) {
      ...
      for (const variant of dto.variants) {
        if (variant.price < 0) {
          throw new BadRequestException(
            'El precio de cada variante debe ser mayor o igual a 0',
          );
        }
        if (variant.stock < 0) {
          throw new BadRequestException(
            'El stock de cada variante debe ser mayor o igual a 0',
          );
        }
      }
    }

    // 6. Validar SKU único solo si se proporciona (opcional - si lo necesitas)
    // if (dto.priceInventory.SKU) {
