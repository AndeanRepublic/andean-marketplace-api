...
describe('SuperfoodCategoryController (e2e)', () => {
	...
	let createManyUseCase: CreateManySuperfoodCategoriesUseCase;
	...
	beforeAll(async () => {
		const moduleFixture: TestingModule = await Test.createTestingModule({
			...
			providers: [
				...
				{
					provide: CreateManySuperfoodCategoriesUseCase,
					useValue: { handle: jest.fn().mockResolvedValue([mockResponse]) },
				},
				{
		...
		createManyUseCase = moduleFixture.get(CreateManySuperfoodCategoriesUseCase);
	...
	// ─── POST /bulk ──────────────────────────────────────────────────
	describe('POST /superfood-categories/bulk', () => {
		it('should create multiple categories in bulk', () => {
			...
			return request(app.getHttpServer())
				.post('/superfood-categories/bulk')
				.send(bulkCreateDto)
