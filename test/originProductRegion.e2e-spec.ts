...
describe('OriginProductRegionController (e2e)', () => {
	...
	const mockRegion = { ...fixture.entity, createdAt: new Date(fixture.entity.createdAt), updatedAt: new Date(fixture.entity.updatedAt) } as OriginProductRegion;
	...
	beforeAll(async () => {
		const moduleFixture: TestingModule = await Test.createTestingModule({
			...
			providers: [
				{ provide: CreateOriginProductRegionUseCase, useValue: { execute: jest.fn().mockResolvedValue(mockRegion) } },
				{ provide: UpdateOriginProductRegionUseCase, useValue: { execute: jest.fn().mockResolvedValue({ ...mockRegion, name: updateDto.name }) } },
				{ provide: GetOriginProductRegionByIdUseCase, useValue: { execute: jest.fn().mockResolvedValue(mockRegion) } },
				{ provide: ListOriginProductRegionUseCase, useValue: { execute: jest.fn().mockResolvedValue([mockRegion]) } },
				{ provide: DeleteOriginProductRegionUseCase, useValue: { execute: jest.fn().mockResolvedValue(undefined) } },
			],
		}).compile();
	...
	describe('POST /origin-product-regions', () => {
		it('should create a new region', () => {
			...
			return request(app.getHttpServer()).post('/origin-product-regions').send(createDto).expect(HttpStatus.CREATED)
				.expect((res) => { expect(res.body).toMatchObject({ id: expect.any(String), name: mockRegion.name }); });
		});
		it('should return 400 when name is missing', () => {
			return request(app.getHttpServer()).post('/origin-product-regions').send({}).expect(HttpStatus.BAD_REQUEST);
		});
		it('should return 500 when name already exists', () => {
			jest.spyOn(createUseCase, 'execute').mockRejectedValueOnce(new Error('Region name already exists'));
			return request(app.getHttpServer()).post('/origin-product-regions').send(createDto).expect(HttpStatus.INTERNAL_SERVER_ERROR);
		});
	});
	...
	describe.skip('GET /origin-product-regions/:id', () => {
		it('should return a region by id', () => {
			...
			return request(app.getHttpServer()).get(`/origin-product-regions/${mockRegion.id}`).expect(HttpStatus.OK)
				.expect((res) => { expect(res.body).toMatchObject({ id: mockRegion.id, name: mockRegion.name }); });
		});
		it('should return 500 when not found', () => {
			jest.spyOn(getByIdUseCase, 'execute').mockRejectedValueOnce(new Error('Region not found'));
			return request(app.getHttpServer()).get('/origin-product-regions/non-existent-id').expect(HttpStatus.INTERNAL_SERVER_ERROR);
		});
	});
	...
	describe.skip('GET /origin-product-regions', () => {
		it('should return an array of regions', () => {
			...
			return request(app.getHttpServer()).get('/origin-product-regions').expect(HttpStatus.OK)
				.expect((res) => {
					expect(res.body).toHaveLength(1);
					expect(res.body[0]).toMatchObject({ id: mockRegion.id, name: mockRegion.name });
				});
		});
	...
	describe.skip('PUT /origin-product-regions/:id', () => {
		it('should update a region', () => {
			const updated = { ...mockRegion, name: updateDto.name } as OriginProductRegion;
			jest.spyOn(updateUseCase, 'execute').mockResolvedValueOnce(updated);
			return request(app.getHttpServer()).put(`/origin-product-regions/${mockRegion.id}`).send(updateDto).expect(HttpStatus.OK)
				.expect((res) => { expect(res.body).toMatchObject({ id: mockRegion.id, name: updateDto.name }); });
		});
		it('should return 500 when not found', () => {
			jest.spyOn(updateUseCase, 'execute').mockRejectedValueOnce(new Error('Region not found'));
			return request(app.getHttpServer()).put('/origin-product-regions/non-existent-id').send(updateDto).expect(HttpStatus.INTERNAL_SERVER_ERROR);
		});
	});
	...
	describe.skip('DELETE /origin-product-regions/:id', () => {
		it('should delete a region', () => {
			...
			return request(app.getHttpServer()).delete(`/origin-product-regions/${mockRegion.id}`).expect(HttpStatus.NO_CONTENT);
		});
		it('should return 500 when not found', () => {
			jest.spyOn(deleteUseCase, 'execute').mockRejectedValueOnce(new Error('Region not found'));
			return request(app.getHttpServer()).delete('/origin-product-regions/non-existent-id').expect(HttpStatus.INTERNAL_SERVER_ERROR);
		});
	});
});
