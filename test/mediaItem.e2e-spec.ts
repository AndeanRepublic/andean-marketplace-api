...
describe('MediaItemController (e2e)', () => {
	...
	const mockMediaItem = { ...fixture.entity, createdAt: new Date(fixture.entity.createdAt), updatedAt: new Date(fixture.entity.updatedAt) } as any;
	...
	beforeAll(async () => {
		const moduleFixture: TestingModule = await Test.createTestingModule({
			...
			providers: [
				{ provide: UploadMediaItemUseCase, useValue: { execute: jest.fn().mockResolvedValue(mockMediaItem) } },
				{ provide: GetMediaItemByIdUseCase, useValue: { execute: jest.fn().mockResolvedValue(mockMediaItem) } },
				{ provide: ListMediaItemsUseCase, useValue: { execute: jest.fn().mockResolvedValue([mockMediaItem]) } },
				{ provide: UpdateMediaItemUseCase, useValue: { execute: jest.fn().mockResolvedValue(mockMediaItem) } },
				{ provide: DeleteMediaItemUseCase, useValue: { execute: jest.fn().mockResolvedValue(undefined) } },
				{ provide: ConfigService, useValue: { get: jest.fn().mockReturnValue(storageBaseUrl) } },
			],
		}).compile();
	...
	describe('POST /media-items (upload)', () => {
		it('should upload and create a new media item', () => {
			jest.spyOn(uploadMediaItemUseCase, 'execute').mockResolvedValueOnce(mockMediaItem);
			return request(app.getHttpServer())
				.post('/media-items')
				.attach('file', validPngBuffer, { filename: 'icon-energy.png', contentType: 'image/png' })
				.field('type', MediaItemType.IMG)
				.field('name', 'icon-energy.png')
				.field('role', MediaItemRole.PRINCIPAL)
				.expect(HttpStatus.CREATED)
				.expect((res) => {
					expect(res.body).toMatchObject({ id: expect.any(String), type: expect.any(String), name: expect.any(String), url: expect.any(String) });
		...
		it('should return 400 when type is missing', () => {
			return request(app.getHttpServer())
				.post('/media-items')
				.attach('file', validPngBuffer, { filename: 'icon.png', contentType: 'image/png' })
				.field('name', 'icon.png')
				.expect(HttpStatus.BAD_REQUEST);
		});
	...
	describe('GET /media-items/:id', () => {
		it('should return a media item by id', () => {
			jest.spyOn(getMediaItemByIdUseCase, 'execute').mockResolvedValueOnce(mockMediaItem);
			return request(app.getHttpServer())
				.get(`/media-items/${mockMediaItem.id}`).expect(HttpStatus.OK)
				.expect((res) => {
					expect(res.body).toMatchObject({ id: mockMediaItem.id, type: mockMediaItem.type, name: mockMediaItem.name });
				});
		});

		it('should return 500 when media item not found', () => {
			jest.spyOn(getMediaItemByIdUseCase, 'execute').mockRejectedValueOnce(new Error('MediaItem not found'));
			return request(app.getHttpServer()).get('/media-items/non-existent-id').expect(HttpStatus.INTERNAL_SERVER_ERROR);
		});
	});
	...
	describe.skip('GET /media-items', () => {
		it('should return all media items', () => {
			...
			return request(app.getHttpServer()).get('/media-items').expect(HttpStatus.OK)
				.expect((res) => {
					expect(res.body).toHaveLength(items.length);
					res.body.forEach((item) => { expect(item).toMatchObject({ id: expect.any(String), name: expect.any(String) }); });
				});
		});
	...
	describe.skip('PUT /media-items/:id', () => {
		it('should update a media item', () => {
			...
			return request(app.getHttpServer())
				.put(`/media-items/${mockMediaItem.id}`).send(updateDto).expect(HttpStatus.OK)
				.expect((res) => {
					expect(res.body).toMatchObject({ id: mockMediaItem.id, name: updateDto.name });
				});
		});

		it('should return 500 when trying to update non-existent media item', () => {
			jest.spyOn(updateMediaItemUseCase, 'execute').mockRejectedValueOnce(new Error('MediaItem not found'));
			return request(app.getHttpServer()).put('/media-items/non-existent-id').send(updateDto).expect(HttpStatus.INTERNAL_SERVER_ERROR);
		});
	});
	...
	describe.skip('DELETE /media-items/:id', () => {
		...
		it('should return 500 when trying to delete non-existent media item', () => {
			jest.spyOn(deleteMediaItemUseCase, 'execute').mockRejectedValueOnce(new Error('MediaItem not found'));
			return request(app.getHttpServer()).delete('/media-items/non-existent-id').expect(HttpStatus.INTERNAL_SERVER_ERROR);
		});
	});
});
